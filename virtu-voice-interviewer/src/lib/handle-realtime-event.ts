...
export default function handleRealtimeEvent(
  ...
  function updateOrAddItem(id: string, updates: Partial<Item>): void {
    setItems((prev) => {
      ...
      if (idx >= 0) {
        const updated = [...prev];
        updated[idx] = { ...updated[idx], ...updates } as Item;
        return updated;
      } else {
  ...
  switch (type) {
        ...
        setItems((prev) => {
          ...
          if (idx >= 0) {
            ...
            updated[idx] = {
              ...
              status: "completed",
              timestamp: updated[idx].timestamp || new Date().toLocaleTimeString(),
            } as Item;
            return updated;
          } else {
            return [
              ...prev,
              createNewItem({ ...item, content: updatedContent, status: "completed" }),
            ];
          }
        });
        ...
        setItems((prev) => {
          const newItems = [
            ...
            createNewItem({
              ...
              role: "tool",
              content: [
                { type: "text", text: `Function call response: ${item.output}` },
              ],
              status: "completed",
            }),
          ];
    ...
    case "conversation.item.input_audio_transcription.delta": {
      ...
      if (delta) {
        setItems((prev) => {
          const idx = prev.findIndex((m) => m.id === item_id && m.role === "user");
          if (idx >= 0) {
            const updated = [...prev];
            const existing = updated[idx].content || [];
            updated[idx] = {
              ...updated[idx],
              content: [...existing, { type: "text", text: delta }],
            } as Item;
            return updated;
          }
          // If not found create running message
          return [
            ...prev,
            createNewItem({
              id: item_id,
